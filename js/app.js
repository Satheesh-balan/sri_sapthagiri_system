...
function printChallan(id) {
    ...
    printWindow.document.write(`
        ...
        <style>
            ...
            body { font-family: 'Inter', sans-serif; padding: 40px; color: #000; font-weight: 700; line-height: 1.4; }
            ...
            .challan-title { font-size: 34px; font-weight: 800; text-align: center; margin: 30px 0; text-decoration: underline; text-transform: uppercase; }
            .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 40px; margin-bottom: 40px; }
            .info-box { border: 2px solid #000; padding: 15px; border-radius: 4px; }
            .info-label { font-weight: 800; color: #000; font-size: 13px; text-transform: uppercase; margin-bottom: 5px; opacity: 0.8; }
            table { width: 100%; border-collapse: collapse; margin-top: 20px; border: 2px solid #000; }
            th, td { border: 2px solid #000; padding: 12px; text-align: left; font-weight: 700; }
            th { background-color: #eee; text-transform: uppercase; font-size: 14px; }
            .footer { margin-top: 120px; display: flex; justify-content: space-between; }
            .sign-line { border-top: 2px solid #000; width: 220px; text-align: center; padding-top: 10px; font-weight: 800; text-transform: uppercase; font-size: 14px; }
        </style></head><body>
        <div class="header"><div class="logo-text">SRI SAPTHAGIRI SYSTEMS</div><div style="font-size:18px;">Date: ${new Date(req.date).toLocaleDateString()}</div></div>
        <div class="challan-title">TRANSPORT CHALLAN</div>
        <div class="info-grid">
            <div class="info-box"><div class="info-label">Source Godown</div><div style="font-size:20px;font-weight:800;">${req.source}</div></div>
            <div class="info-box"><div class="info-label">Destination</div><div style="font-size:20px;font-weight:800;">${req.dest}</div></div>
        </div>
        <table>
            <thead><tr><th>Category</th><th>Product</th><th>Serial Number</th><th>Quantity</th></tr></thead>
            <tbody>
                ${items.map(item => `
                    <tr>
                        <td>${item.category}</td>
        ...
        <div class="footer"><div class="sign-line">Transporter Signature</div><div class="sign-line">Admin Signature</div></div>
